...
RUN export TOKEN=$(curl -s https://download.cobaltstrike.com/download -d "dlkey=${COBALTSTRIKE_LICENSE}" | grep 'href="/downloads/' | cut -d '/' -f3,4) && \
    cd /opt && \
...
EXPOSE 50050 50443 80 443 4443 4444 4445 4446 4447 4448 4449 9050 9051 9053 9054 9055 9056 53/udp

# Set the working directory to the server directory
WORKDIR /opt/cobaltstrike/server

# Set the entry point to start both teamserver and csrestapi.
# Arguments for the teamserver (IP, password, profile) should still be passed
# directly when running the container.
